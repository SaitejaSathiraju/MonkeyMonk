...
const Dashboard: React.FC = () => {
  ...
  const [totalFeaturedPlaces, setTotalFeaturedPlaces] = useState<number>(0);

  useEffect(() => {
    // Function to fetch total users
    const fetchTotalUsers = async () => {
      // Check if total users are stored in local storage
    ...
    // Function to fetch total posts
    const fetchTotalPosts = async () => {
      // Check if total posts are stored in local storage
      const storedTotalPosts = localStorage.getItem('totalPosts');
      if (storedTotalPosts) {
        setTotalPosts(parseInt(storedTotalPosts));
      } else {
        const postsSnapshot = await getDocs(collection(db, 'trips'));
        const totalPostsCount = postsSnapshot.size;
        setTotalPosts(totalPostsCount);
        localStorage.setItem('totalPosts', totalPostsCount.toString());
      }
    };

    // Function to fetch total blogs
    const fetchTotalBlogs = async () => {
      // Check if total blogs are stored in local storage
      const storedTotalBlogs = localStorage.getItem('totalBlogs');
      if (storedTotalBlogs) {
    ...
    // Function to fetch total featured places
    const fetchTotalFeaturedPlaces = async () => {
      // Check if total featured places are stored in local storage
      ...
      if (storedTotalFeaturedPlaces) {
        ...
        localStorage.setItem(
          'totalFeaturedPlaces',
          totalFeaturedPlacesCount.toString(),
        );
      }
    };
  ...
  return (
    <DefaultLayout>
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        ...
        <CardDataStats title="Total posts" total={totalPosts}>
        <svg
                    className="fill-primary"
                    width="18"
                    height="19"
        ...
        <CardDataStats title="Available blogs" total={totalBlogs}>
        <svg
                    className="fill-primary"
                    width="18"
                    height="19"
        ...
        <CardDataStats title="Featured Places" total={totalFeaturedPlaces}>
          <svg
